...
entropyItoProcess <- function(t, s, dynamics, rate = 0)
{
  # setupPDE handles the time-reversion, so no need to here.
  mu <- function(tt, ss) dynamics[[1]](tt, ss)
  volat <- function(tt, ss) dynamics[[2]](tt, ss)
  sharpe <- function(tt, ss) (mu(tt, ss)- rate)/volat(tt, ss)

  priceDynamics <- list(function(tt, ss) mu(tt, ss)*ss,
                   function(tt, ss) volat(tt, ss)*ss
  )
  problem <- list(function(tt, ss) 0,
                  function(tt, ss) rate+0.5*sharpe(tt, ss)^2,
                  function(x) 0
  )
  region <- c(t, 0, 2*s)
  control <- list(N = 500, M = 500, variational = FALSE, output = "price", engine = "c++")
  v <- fkpde::solvePDE(priceDynamics, problem, region, control)
  return(v)
...
optimalItoProcess <- function(bankroll, t, spot, rate, dynamics)
{
  ...
  z <- sdes::samplePathSystem(f, g, IC, t0 = 0, tn = t, n = 1000)
  sol <- z
